...
// Forward declarations for CA certificates (defined below setupAnalyzerClients)
...
void MQTTBridge::begin() {
  ...
  // Set CA cert for TLS verification (chicagooffline uses Let's Encrypt)
...
bool MQTTBridge::isMQTTConfigValid() {
  ...
  // In WS mode, JWT auth is used (same pattern as LetsMesh analyzers)
  // so username/password prefs are not required
  if (_prefs->mqtt_ws) {
    MQTT_DEBUG_PRINTLN("Custom broker using WS mode — JWT auth will be used");
    return true;
  }
  
  // For non-WS mode, require username/password
...
// ISRG Root X1 - Let's Encrypt RSA root (for chimesh.org + chicagooffline.com)
...
void MQTTBridge::setupAnalyzerClients() {
  ...
  if (_analyzer_eu_enabled) {
    ...
    _analyzer_eu_client->setCACert(ISRG_ROOT_X1);  // chimesh.org uses Let's Encrypt
